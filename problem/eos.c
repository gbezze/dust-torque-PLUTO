  ...
  #elif EOS == ISOTHERMAL
  {
    ...
    if (g_dir == IDIR) {
      ...
      for (i = beg; i <= end; i++){
         ...
         R = x1[i]*sin(x2[j]);
        #endif
        p->a2[i] = g_isoSoundSpeed*g_isoSoundSpeed*pow(R,-g_inputParam[Q_INDEX]);
      }
      ...
      for (j = beg; j <= end; j++) {
         ...
         R = x1[i]*sin(x2[j]);
        #endif
        p->a2[j] = g_isoSoundSpeed*g_isoSoundSpeed*pow(R,-g_inputParam[Q_INDEX]);
      }
    }else if (g_dir == KDIR){
      double R;
      ...
      for (k = beg; k <= end; k++){
         ...
         R = x1[i]*sin(x2[j]);
        #endif
        p->a2[k] = g_isoSoundSpeed*g_isoSoundSpeed*pow(R,-g_inputParam[Q_INDEX]);
      }
    }
  }
  #else
